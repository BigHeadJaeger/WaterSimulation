...
vector<double> MPSToolFun::ImplicitCalculateP(vector<vec3>& r, float n0, vector<bool>& isSurface, vector<double> Right)
{
	//每一行中的n0和lambda都是不同的
	...
	for (int i = 0; i < r.size(); i++)
	{
		...
		double con = 2 * Ds / (n0 * testLambda0);	//每一行的常量
		//double con = 2 * Ds / (DensityN(r,i) * lambda);	//每一行的常量
...
{
	return (0.2 * rho0 / pow(deltaT, 2)) * ((n0 - tempN) / n0);
...
mat3 MPSToolFun::GetMaterixC(vector<vec3>& R, int currentIndex, float n0)
{
	...
	for (int i = 0; i < R.size(); i++)
	{
		if (i != currentIndex)
		{
			...
			res += (WeightFun(length(R[i] - R[currentIndex]), reForG) * outerProduct(v1, v1));
		}
	}
	res *= (1 / n0);
	return res;
	...
	if (determinant(C) >= 0.05)
	{
		...
		for (int i = 0; i < p.size(); i++)
		{
			if (i != currentIndex)
			{
				...
				res += (WeightFun(length(l), reForG) * (((float)p[i] - (float)p[currentIndex]) / l) * ((r[i] - r[currentIndex]) / l));
			}
		}
		res /= n0;
...
vec3 MPSToolFun::OldGradient(vector<vec3>& r, vector<double>& p, int currentIndex, float n0)
{
	vec3 res(0);
	for (int i = 0; i < r.size(); i++)
	{
		if (i!=currentIndex)
		{
			float dis = distance(r[i], r[currentIndex]);
			res += (((float)p[i] - (float)p[currentIndex]) / (dis * dis)) * WeightFun(dis, reForG) * (r[i] - r[currentIndex]);
		}
	}
...
float MPSToolFun::DensityN(vector<vec3>& r, int currentIndex)
{
	...
	for (int i = 0; i < r.size(); i++)
	{
		if (i != currentIndex)
		{
			res += WeightFun(distance(r[i], r[currentIndex]), reForD);
		}
	}
	return res;
}
